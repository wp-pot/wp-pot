...
describe('JavaScript', () => {
  describe('Function tests', () => {
    ...
    it('Can merge duplicate strings and separate with context', () => {
      const fixturePaths = [
        'test/fixtures/duplicated-strings-1.js',
        'test/fixtures/duplicated-strings-2.js'
      ];
      ...
      for (const fixturePath of fixturePaths) {
        assert(testHelper.verifyLanguageBlock(potContents, false, fixturePath + ':1', 'Simple string', false, false));
        assert(testHelper.verifyLanguageBlock(potContents, false, fixturePath + ':2', 'Simple string', false, false));
        assert(testHelper.verifyLanguageBlock(potContents, false, fixturePath + ':3', 'Simple string', false, 'with context'));
        assert(testHelper.verifyLanguageBlock(potContents, false, fixturePath + ':4', 'Simple string', false, 'with context'));
        assert(testHelper.verifyLanguageBlock(potContents, false, fixturePath + ':5', 'Single and plural string', 'Plural string', false));
        assert(testHelper.verifyLanguageBlock(potContents, false, fixturePath + ':6', 'Single and plural string', 'Plural string', false));
        assert(testHelper.verifyLanguageBlock(potContents, false, fixturePath + ':7', 'Single and plural string', 'Plural string', 'with context'));
        assert(!testHelper.verifyLanguageBlock(potContents, false, fixturePath + ':8', 'Simple string', false, false));
      }
    });
  });
});
